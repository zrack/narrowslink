...
2. Load a candidate `.nlsession`, `.json`, or `.nlb`. NarrowsLink validates sessions through the normal session and decoder pipeline and verifies bundles through the production receiver before allowing comparison; choose the candidate incident when a whole session was loaded.
3. Declare range-start alignment or a named shared event with exact baseline and candidate microsecond anchors. NarrowsLink computes only the intersecting relative interval and reports unmatched tails.
